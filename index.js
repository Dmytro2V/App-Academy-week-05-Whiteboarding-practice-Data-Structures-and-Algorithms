...
will return an array from lowNum to highNum, inclusive. 
Inclusive means that the range includes 
lowNum and highNum.
...
// can use Array(highNum - lowNum) instead of push
console.log('logBetween: O(n), S(n), optimal');
...
console.log('logBetweenStepper: O(n), S(n), optimal');
...
console.log('printReverse: O(n), S(n), optimal');
...
returns an array of every number
from 0 to max that is divisible by either 3 or 5, but not both.

Examples:
*/
// probably can be some optimised if loops on 3 and 5 and compare, but not sure
...
console.log('fizzBuzz: O(n), S(n), mostly optimal');
...
console.log('isPrime: O(n), S(1), mostly optimal');
...
console.log('maxValue: O(n), S(1), mostly optimal');
...
/* 7 myIndexOf
Write a function myIndexOf(array, target) that takes in an array of numbers 
and a target number as arguments. 
It should return the index value of the target if it is present in the array 
or -1 if it is not present.

letRAINT: Do not use the indexOf or includes method
...
console.log('myIndexOf: O(n), S(1), optimal');
...
console.log('factorArray: O(n), S(n), optimal?');
...
/* 9 oddRange
Write a function oddRange(end) that takes in a number and returns 
an array containing all positive odd numbers up to end.
...
console.log('oddRange: O(n), S(n), optimal');
...
function mirrorArray(array) {
  console.log(array.concat(array.slice().reverse()))
...
returns a new sentence where words longer than 4 characters 
have their vowels removed. Assume the sentence has all lowercase characters. 
...
function abbreviate(sentence) {
  ...
  let abbrWords = words.map(word => {
    if (word.length <= 4) return word;

    let letters = word.split('');
    let lettersFiltered = letters.filter(letter => !vowels.includes(letter))
    return lettersFiltered.join('')
  })
  let abbrSentence = abbrWords.join(' ');
...
function adults(people) {
  let ret = people.filter(person => person.age >= 18).map(person => person.name)
  console.log(ret)
  return ret
}
...
an object that has key-value pairs where each name is a key and the value 
is their total score.
*/
function countScores(people) {   
  let ret = {}
  for (let obj of people) {
    if (ret[obj.name] === undefined) ret[obj.name] = obj.score;
    else ret[obj.name] += obj.score;
  }
  console.log(ret);
...
function peakFinder(array) {
  let ret = array.map((n, i, arr) => {
    if ( (i === 0 && n > arr[i + 1]) ||
         (i === arr.length - 1 && n > arr[i - 1]) ||
         (n> arr[i-1] && n > arr[i+1]) ) return i
    else return null
  })
...
It should return an array of all the pairs of indices, whose elements 
...
function divisibleByThreePairSum(array) {
  ...
  for (let i = 0; i < array.length; i++) {
    for (j = i + 1; j < array.length; j++) {
      if ((array[i] + array[j]) % 3 === 0) ret.push([i, j]);
    }
  } 
  console.log(ret);
  return ret;
...
/* 19. zipArray
Write a function zipArray(arr1, arr2) that takes in two arrays and "zips" 
...
function zipArray(arr1, arr2) {
  let ret = arr1.map((el1, i) => [el1, arr2[i]])
  console.log(ret);
  return ret;
}
...
Write a function twoDimensionalTotal(array) that takes in a 2D array of 
numbers and returns the total sum of all elements. 
Examples:
*/
function twoDimensionalTotal(array) {
  let ret = array.reduce((accum, arr) => {
    return accum += arr.reduce(((acc, el) => acc += el), 0)
  }, 0)
  console.log(ret);
  return ret
}
...
You should return an object that counts how many times each element 
in each sub array repeats.
...
where each of the sub array 
are indices of the two numbers such that their difference is 2. 
...
function valueCounter(obj, val) {
  ret= Object.values(obj).filter(el => el === val).length
  console.log(ret);
  return ret
}
...
let obj2 = { Anne: 50, Alvin: 1, JJ: 100, Roman: 100 }
valueCounter(obj2, 88)  // => 0

let pairs = { Anne: 'Roman', Alvin: 'Roman', JJ: 'Anne', Roman: 'Anne' }
...
Example:
  // - go on all elements and creates with 0/ adds 1 count
elementCount(["a", "a", "b", "b"]); // => { "a" : 2, "b" : 2 }
elementCount(['c', 'a', 'c', 'a', 'b']); // => { "c": 2, "a": 2, "b": 1 }

26 nextTwoPrimes
Write a function nextTwoPrimes(num) that takes in a number num and returns 
...
and a target number, num. 
It should return pairs of indices whose elements multiply to num. 
No pair should appear twice in the result.

letRAINT: Use only while loops. No for loops.
...
/* 34. objectToString
Write a function objectToString(count) that takes in 
a char count object and 
...
Write a function greatestCommonFactor(num1, num2) that 
returns the largest number that is divides both num1 and num2.
...
37. isPassing
Write a function isPassing(assessments) that 
takes in an array of assessment objects. The function should 
return true if the average assessment score is at least 70. 
...
function isPassing(assessments) { 
  ...
  ret = vals/assessments.length >= 70
  console.log(ret);
  return ret
}
...
The function should return a new array where 
each element is concatenated with it's corresponding value from the object.
...
let arr = ['alex', 'maurice', 'meagan', 'ali'];
let obj = { alex: 'bronca', ali: 'harris' }
...
39. Write a function threeInARow(arr) that takes in an array of numbers and 
...
41. threeIncreasing
Write a function threeIncreasing(arr) that takes in an array of numbers and 
returns true if the array contains 3 consecutive numbers in increasing order. 
The function should return false otherwise. 
Note that the 3 consecutive numbers should be increasing by 1, so 
the the second number is 1 greater than the first, 
...
The function should return a string representing the elements of the array 
in reverse order.
...
44. countRepeats
Write a function countRepeats(string) that takes in a string and returns 
the number of letters that appear more than once in the string. 
You may assume the string contains only lowercase letters. 
Count the number of letters that repeat, 
...
switch signs from positive to negative or vice versa. 
...
base and length. The function should 
return an array containing a power sequence with the given length. 
Assume that the length is at least 1.

The first number of a power sequence begins with base. 
The second number of the sequence is the product between the first number 
and the base. 
The third number of the sequence is the product between the second number 
and the base...
...
The function should return the string where 'streaks' 
of consecutive characters are collapsed to a single character.
...
/*
50.oddWordsOut
Write a function oddWordsOut(sentence) that 
takes in a sentence string and 
returns the sentence where 
words with an odd number of characters are removed.
...
51. mindPsAndQs
Write a function mindPsAndQs(str) that 
accepts a string of uppercase letters. The function should 
return the length of the longest consecutive streak 
of the letters 'P' and 'Q'.

Hint: Use two variables. One to track the length of the 
current streak and another to track the length 
of the longest streak so far. 
...
52. commonFactors
Write a function commonFactors(num1, num2) that 
returns an array that contains the common factors between 
both numbers. 
...
53. commonPrimeFactors // O(n^2)
Write a function commonPrimeFactors(num1, num2) that takes in 
two numbers as arguments and 
returns an array of all prime factors that are common 
between the two numbers. 
A factor is a number that divides another number without 
...
returns two halves of that array split in the middle. 
If the array has an odd number of elements, then 
the middle element should be excluded.
...
Write a function threeUniqueVowels(string) that takes in 
a string and 
returns true if the string contains at least three 
different vowels.
...
56. vowelShift
W3ite a function vowelShift(sentence) that takes in 
a sentence string. The function should 
return a new sentence, where every vowel is replaced 
...
57. hasSymmetry
Write a function hasSymmetry(array) that takes in 
an array. The function should 
return true if the array has symmetry, false otherwise. 
For an array to have symmetry, 
...
58. evenSumArray// O(n*n) - num growth*arr.length
Write a function evenSumArray(array) that takes in 
an array of numbers and 
returns a new array where each num is replaced 
with the sum of all even numbers less than or equal to 
that num.
...
59. numsToWords
Write a function numsToWords(numString) that takes in 
a string representing a number. The function should 
return a new string where each digit character is replaced 
with it's "word" representation. Assume the input string 
...
60. moreDotLessDash
Write a function moreDotLessDash(str) that 
